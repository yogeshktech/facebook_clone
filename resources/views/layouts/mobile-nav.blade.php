<nav class="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 md:hidden safe-area-bottom">
    <div class="flex items-center justify-around h-14 px-1">
        ...
        <a href="{{ route('profile.show', auth()->user()) }}" class="flex items-center gap-2 hover:bg-fb-gray rounded-full p-1 pr-2 sm:pr-3">
                ...
                <span class="hidden md:block text-sm font-medium max-w-[120px] truncate">Profile</span>
            </a>
        ...
        <a href="{{ route('videos.index') }}" class="mobile-nav-icon {{ request()->routeIs('videos.*') ? 'active' : '' }}" title="Videos">
            <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M17 10.5V6c0-1.1-.9-2-2-2H5C3.9 4 3 4.9 3 6v12c0 1.1.9 2 2 2h10c1.1 0 2-.9 2-2v-4.5l4 4v-11l-4 4z"/></svg>
            <span>Videos</span>
        </a>
