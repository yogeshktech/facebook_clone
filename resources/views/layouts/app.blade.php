...
<head>
    ...
    @auth
        {{-- Call UI CSS from /public (live-safe, no Vite dependency) --}}
        @php
            $callUiCss = public_path('css/call-ui.css');
            $callUiCssV = file_exists($callUiCss) ? filemtime($callUiCss) : time();
        @endphp
        <link rel="stylesheet" href="{{ asset('css/call-ui.css') }}?v={{ $callUiCssV }}" id="call-ui-css">
    @endauth
</head>
    ...
    @auth
        ...
        <div id="call-overlay" class="call-overlay hidden" data-call-phase="none" data-call-video="0">
            ...
            <div id="call-videos-container" class="hidden relative w-full max-w-2xl aspect-video bg-slate-950/80 rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 my-4 flex-1 self-center flex items-center justify-center z-10">
                ...
                <!-- PiP (tap to swap) -->
                <div id="pip-video-wrap" class="pip-br group" title="Drag to corner · Tap to swap" role="button" tabindex="0">
