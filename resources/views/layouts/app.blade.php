...
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.css">
    ...
    @auth
        ...
        {{-- Call UI Overlay --}}
        <div id="call-overlay" class="hidden fixed inset-0 z-[100] bg-slate-950/90 flex flex-col items-center justify-between p-6 text-white safe-area-bottom backdrop-blur-2xl">
            <!-- Deep Ambient Background -->
            <div class="absolute inset-0 overflow-hidden pointer-events-none z-0">
                <div class="absolute -top-32 -left-32 w-96 h-96 bg-indigo-500/10 rounded-full blur-[120px] orb-indigo"></div>
                <div class="absolute -bottom-32 -right-32 w-96 h-96 bg-purple-500/10 rounded-full blur-[120px] orb-cyan"></div>
                <div class="absolute inset-0 bg-[radial-gradient(#ffffff02_1px,transparent_1px)] [background-size:16px_16px] opacity-40"></div>
            </div>

            <!-- Top bar: minimize -->
            <button type="button" id="minimize-call-btn" class="hidden absolute top-4 left-4 z-30 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 flex items-center justify-center" title="Minimize">
            ...
            <!-- Call Header / Profile -->
            <div id="call-profile-block" class="w-full max-w-md text-center mt-12 z-10 relative">
                <div class="relative w-32 h-32 mx-auto flex items-center justify-center">
                    <!-- Ripple rings for calling/ringing -->
                    <div class="ripple-ring"></div>
                    <div class="ripple-ring"></div>
                    <div class="ripple-ring"></div>
                    <img id="call-user-avatar" src="" alt="" class="w-28 h-28 rounded-full object-cover border-2 border-white/20 shadow-2xl relative z-10 animate-call-pulse">
                </div>
                <h3 id="call-user-name" class="text-3xl font-bold mt-6 tracking-tight bg-gradient-to-r from-white via-slate-100 to-slate-300 bg-clip-text text-transparent">Username</h3>
                <div class="inline-flex items-center gap-2 mt-3 px-4 py-1.5 rounded-full glass-panel border border-white/5 text-xs font-semibold tracking-wide text-indigo-200">
            ...
            <div class="w-full max-w-lg px-4 py-4 rounded-3xl glass-panel border border-white/5 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] flex justify-center items-center gap-3 sm:gap-5 mb-12 z-20 relative flex-wrap">
                <!-- Decline Button -->
                <button type="button" id="decline-call-btn" class="hidden w-14 h-14 bg-rose-500 hover:bg-rose-600 active:scale-95 text-white rounded-full flex items-center justify-center shadow-lg transition-all duration-200 z-20 animate-heartbeat" title="Decline">
                ...
                <!-- Accept Button -->
                <button type="button" id="accept-call-btn" class="hidden w-14 h-14 bg-emerald-500 hover:bg-emerald-600 active:scale-95 text-white rounded-full flex items-center justify-center shadow-lg transition-all duration-200 z-20 animate-heartbeat" title="Accept">
                ...
                <!-- Mute Button -->
                <button type="button" id="toggle-mute-btn" class="hidden w-12 h-12 bg-white/10 hover:bg-white/20 active:scale-95 text-white rounded-full flex items-center justify-center border border-white/10 shadow-md transition-all duration-200" title="Mute Audio">
                ...
                <button type="button" id="speaker-btn" class="hidden w-12 h-12 bg-white/10 hover:bg-white/20 active:scale-95 text-white rounded-full flex items-center justify-center border border-white/10 shadow-md transition-all duration-200" title="Audio output">
                ...
                <button type="button" id="toggle-video-btn" class="hidden w-12 h-12 bg-white/10 hover:bg-white/20 active:scale-95 text-white rounded-full flex items-center justify-center border border-white/10 shadow-md transition-all duration-200" title="Toggle Camera">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <!-- Flip Camera Button -->
                <button type="button" id="flip-camera-btn" class="hidden w-12 h-12 bg-white/10 hover:bg-white/20 active:scale-95 text-white rounded-full flex items-center justify-center border border-white/10 shadow-md transition-all duration-200" title="Flip Camera">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <!-- Hangup Button -->
                <button type="button" id="hangup-call-btn" class="hidden w-14 h-14 bg-rose-500 hover:bg-rose-600 active:scale-95 text-white rounded-full flex items-center justify-center shadow-lg transition-all duration-200 z-20" title="End Call">
        ...
        {{-- Minimized floating call bubble (background call) --}}
        <div id="call-minimized" class="hidden fixed bottom-20 right-4 z-[110] w-36 sm:w-44 rounded-2xl overflow-hidden shadow-2xl border border-white/20 bg-slate-950 text-white cursor-pointer select-none call-minimized-bubble">
            <div class="relative aspect-[3/4] bg-slate-900">
                <video id="mini-call-video" autoplay playsinline muted class="w-full h-full object-cover"></video>
                <div id="mini-call-avatar-wrap" class="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-slate-900">
                    <img id="mini-call-avatar" src="" alt="" class="w-14 h-14 rounded-full object-cover border-2 border-white/20">
                    <span id="mini-call-name" class="text-xs font-semibold px-2 text-center truncate max-w-full">Call</span>
                </div>
                <div id="mini-remote-muted" class="hidden absolute top-2 left-2 w-7 h-7 rounded-full bg-black/70 flex items-center justify-center">
                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"/><path stroke-linecap="round" stroke-linejoin="round" d="M3 3l18 18"/></svg>
                </div>
                <div class="absolute bottom-0 inset-x-0 p-2 flex items-center justify-between bg-gradient-to-t from-black/80 to-transparent">
                    <span id="mini-call-status" class="text-[10px] font-medium text-emerald-300">In call</span>
                    <button type="button" id="mini-hangup-btn" class="w-8 h-8 rounded-full bg-rose-500 hover:bg-rose-600 flex items-center justify-center" title="End Call">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 9c-2.2 0-4.3.4-6.2 1.1-.6.2-1 .7-1.3 1.2l-1.2 1.2c-.5.5-.6 1.2-.3 1.8 1.1 2.2 2.7 4.1 4.7 5.5.5.4 1.2.3 1.6-.2l1.2-1.2c.4-.4.5-.9.3-1.4-.4-1.2-.6-2.4-.6-3.7v-.4c3.4-.6 6.9-.6 10.3 0v.4c0 1.3-.2 2.5-.6 3.7-.2.5-.1 1.1.3 1.4l1.2 1.2c.4.5 1.1.6 1.6.2 2-1.4 3.6-3.3 4.7-5.5.3-.6.2-1.3-.3-1.8l-1.2-1.2c-.3-.5-.7-1-1.3-1.2C16.3 9.4 14.2 9 12 9z"/></svg>
                    </button>
                </div>
            </div>
        </div>
