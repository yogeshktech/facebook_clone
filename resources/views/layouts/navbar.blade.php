    ...
    <div class="max-w-7xl mx-auto w-full flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div class="flex items-center gap-2 flex-1 min-w-0">
            ...
            <form action="{{ route('search') }}" method="GET" class="hidden md:block">
                <input type="search" name="q" value="{{ request('q') }}" placeholder="Search NEWBOOK" class="bg-fb-gray rounded-full px-4 py-2 w-60 text-sm focus:outline-none focus:ring-2 focus:ring-fb-blue">
            </form>
            <a href="{{ route('search') }}" class="md:hidden nav-icon flex-shrink-0" title="Search">
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0016 9.5 6.5 6.5 0 109.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C8.01 14 6 11.99 6 9.5S8.01 5 10.5 5 15 7.01 15 9.5 12.99 14 10.5 14z" /></svg>
            </a>
        </div>
        ...
        <div class="hidden md:flex items-center gap-1">
            ...
            <a href="{{ route('videos.index') }}" class="nav-icon {{ request()->routeIs('videos.*') ? 'active' : '' }}" title="Videos">
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            ...
            <button type="button" onclick="openMobileMenu()" class="flex items-center gap-2 hover:bg-fb-gray rounded-full p-1 pr-2 sm:pr-3 transition" title="Menu">

                <img src="{{ auth()->user()->avatar_url }}" alt="" class="w-8 h-8 rounded-full object-cover">

                <span class="hidden md:block text-sm font-medium max-w-[120px] truncate">
                    {{ auth()->user()->name }}
                </span>
            </button>
