...
<div class="max-w-7xl mx-auto px-4 py-8">
    <div class="mb-6">
        <a href="{{ route($backRoute ?? 'ads.index') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition flex items-center gap-1">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            Back to {{ ($backRoute ?? 'ads.index') === 'admin.ads.index' ? 'Admin Panel' : 'Dashboard' }}
        </a>
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-2">
            ...
            @if(!$leads->isEmpty())
                <a href="{{ route($downloadRoute ?? 'ads.leads.download', $ad) }}" class="btn-primary flex items-center gap-2 text-sm">
    ...
    @if($leads->isEmpty())
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p class="text-gray-500 max-w-md mx-auto">Once your ad is paid and running on the feed, users who click it can submit their details here.</p>
        </div>
    @else
